...
class CreateBattleService
  ...
  def generate_battle_result(character_a, character_b)
    prompt = <<~PROMPT
      ...
      この2体のキャラクターのやり取りの物語を考え、event、resultを必ずJSON形式のみで返してください。
      これらはすべて必須項目です。
      キャラAについては選択されたスキル「#{character_a[:skill][:name]}」のみを使用してください。
      キャラBについては選択されたスキル「#{character_b[:skill][:name]}」のみを使用してください。
      eventはやり取りの内容を、物語形式で1000~1300文字の文章で日本語で書いてください。半分が戦闘、もう半分が戦闘前または後のやり取りといった構成にし、予想外で面白い内容にしてください。
      resultは、eventに基づいて勝ったキャラを「A」または「B」として返してください。
